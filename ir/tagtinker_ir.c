/**
 * ESLPwn IR transmitter - implementation (v2)
 *
...
void eslpwn_ir_init(void) {
    ...
    FURI_LOG_I("ESLPwn", "IR TX initialized: carrier %.3f MHz",
        (double)(64000000.0f / (CARRIER_ARR + 1) / 1000000.0f));
}

void eslpwn_ir_deinit(void) {
    if(!ir_initialized) return;
    ...
    FURI_LOG_I("ESLPwn", "IR TX deinitialized");
}

bool eslpwn_ir_transmit(const uint8_t* data, size_t len, uint16_t repeats_raw, uint8_t delay) {
    ...
    FURI_LOG_I("ESLPwn", "TX start: %zu bytes, %lu repeats (PP%d), %u delay",
        len, repeats, is_pp16 ? 16 : 4, delay);

    /* Transmit frame with repeats.
     * Between repeats we yield briefly so FreeRTOS stays happy
     * and the user can cancel via eslpwn_ir_stop(). */
    for(uint32_t rep = 0; rep <= repeats; rep++) {
        if(ir_stop_requested) {
            FURI_LOG_I("ESLPwn", "TX cancelled at repeat %lu", rep);
            carrier_off();
            return false;
        }
    ...
    FURI_LOG_I("ESLPwn", "TX complete");
    return true;
}

bool eslpwn_ir_is_busy(void) {
    return false; /* Transmit is blocking in this version */
}

void eslpwn_ir_stop(void) {
