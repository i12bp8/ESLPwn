    ...
    do {
        if(app->frame_seq_count > 0) {
            for(size_t i = 0; i < app->frame_seq_count; i++) {
                ...
                ok = eslpwn_ir_transmit(
                    app->frame_sequence[i], app->frame_lengths[i],
                    app->frame_repeats[i], 10);
                if(!ok) break;
            }
        } else {
            ok = eslpwn_ir_transmit(app->frame_buf, app->frame_len, app->repeats, 10);
        }
...
void eslpwn_scene_transmit_on_enter(void* context) {
    EslPwnApp* app = context;
    ...
    view_dispatcher_switch_to_view(app->view_dispatcher, EslPwnViewTransmit);
...
bool eslpwn_scene_transmit_on_event(void* context, SceneManagerEvent event) {
    EslPwnApp* app = context;
    ...
    if(event.type == SceneManagerEventTypeBack) {
        if(app->tx_active) {
            ...
            if(!scene_manager_search_and_switch_to_previous_scene(app->scene_manager, EslPwnSceneTargetActions)) {
                if(!scene_manager_search_and_switch_to_previous_scene(app->scene_manager, EslPwnSceneBroadcast)) {
                    if(!scene_manager_search_and_switch_to_previous_scene(app->scene_manager, EslPwnSceneBroadcastMenu)) {
                        scene_manager_search_and_switch_to_previous_scene(app->scene_manager, EslPwnSceneMainMenu);
                    }
                }
            }
            return true;
...
void eslpwn_scene_transmit_on_exit(void* context) {
    EslPwnApp* app = context;
