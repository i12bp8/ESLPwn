...
static void presets_load(EslPwnApp* app) {
    app->preset_count = 0;
    ...
    if(storage_file_open(file, APP_DATA_PATH("presets.txt"),
        ...
        char* line = buf;
        while(line && *line && app->preset_count < ESLPWN_MAX_PRESETS) {
            char* nl = strchr(line, '\n');
            if(nl) *nl = '\0';
            ...
            if(sscanf(line, "%u|%u|%u|%u|%u|", &w, &h, &pg, &inv, &clr) == 5) {
                ...
                memset(app->presets[idx].text, 0, ESLPWN_PRESET_TEXT_LEN);
                if(pipes == 5) {
                    strncpy(app->presets[idx].text, p, ESLPWN_PRESET_TEXT_LEN - 1);
                }
            }
            line = nl ? nl + 1 : NULL;
        }
...
static char preset_labels[ESLPWN_MAX_PRESETS][48];
...
bool eslpwn_scene_preset_list_on_event(void* ctx, SceneManagerEvent event) {
    ...
    if(event.event == EVT_ADD_NEW) {
        ...
        scene_manager_set_scene_state(app->scene_manager, EslPwnSceneTextInput, 0);
        scene_manager_next_scene(app->scene_manager, EslPwnSceneTextInput);
        return true;
    }
    ...
    if(idx < app->preset_count) {
        ...
        FURI_LOG_I(ESLPWN_TAG, "Preset %lu: %ux%u \"%s\"",
