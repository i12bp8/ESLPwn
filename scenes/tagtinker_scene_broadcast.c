...
void eslpwn_scene_broadcast_on_enter(void* context) {
    EslPwnApp* app = context;
    ...
    if(app->broadcast_type == EslPwnBroadcastFlipPage) {
        /* Page selector 0-7 */
...
bool eslpwn_scene_broadcast_on_event(void* context, SceneManagerEvent event) {
    EslPwnApp* app = context;
    bool consumed = false;
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(app->broadcast_type == EslPwnBroadcastFlipPage) {
            ...
            app->frame_len = eslpwn_build_broadcast_page_frame(
                app->frame_buf, app->page, app->forever, app->duration);
        } else {
            /* Build debug broadcast frame */
            app->frame_len = eslpwn_build_broadcast_debug_frame(app->frame_buf);
        }
...
void eslpwn_scene_broadcast_on_exit(void* context) {
    EslPwnApp* app = context;
