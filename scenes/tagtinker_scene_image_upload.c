...
#include "../views/tagtinker_test_image.h"

static void show_error_dialog(EslPwnApp* app, const char* text) {
...
void eslpwn_scene_image_upload_on_enter(void* ctx) {
    ...
    if(app->use_hardcoded_image) {
        uint16_t w = 128;
        uint16_t h = 64;

        /* Prepare frame sequence for test pattern */
        app->frame_sequence = malloc(sizeof(uint8_t*) * 512);
        app->frame_lengths  = malloc(sizeof(size_t) * 512);
        app->frame_repeats  = malloc(sizeof(uint16_t) * 512);
        for(int k = 0; k < 512; k++) {
            app->frame_sequence[k] = malloc(ESLPWN_MAX_FRAME_SIZE);
        }

        eslpwn_build_image_sequence(app, target->plid, test_image_data, w, h, 0, 0, 0, 250);
            ...
            if(storage_file_read(file, header, sizeof(header)) == sizeof(header)) {
                if(header[0] == 'B' && header[1] == 'M') {
                    ...
                    if(bpp == 1) {
                        ...
                        eslpwn_build_image_sequence(app, target->plid, img_data, (uint16_t)w, (uint16_t)h, 0, 0, 0, 250);
